# 📝 Supported AddOns List

This repository provides **Spanish (es_ES/es_MX) translations** for the following World of Warcraft AddOns:

| AddOn Name | Folder | Status |
|------------|--------|---------|
| **Altoholic** | `Altoholic` | 🛑 |
| **Altoholic Achievements** | `Altoholic_Achievements` | 🛑 |
| **Altoholic Agenda** | `Altoholic_Agenda` | 🛑 |
| **Altoholic Characters** | `Altoholic_Characters` | 🛑 |
| **Altoholic Grids** | `Altoholic_Grids` | 🛑 |
| **Altoholic Guild** | `Altoholic_Guild` | 🛑 |
| **Altoholic Options** | `Altoholic_Options` | 🛑 |
| **Altoholic Search** | `Altoholic_Search` | 🛑 |
| **Altoholic Summary** | `Altoholic_Summary` | 🛑 |
| **Baganator** | `Baganator` | ✅ |
| **Betteraddonlist** | `BetterAddonList` | 🛑 |
| **Betterwardrobe** | `BetterWardrobe` | 🛑 |
| **Btwquests** | `BtWQuests` | 🛑 |
| **Btwquestsbattleforazeroth** | `BtWQuestsBattleForAzeroth` | 🛑 |
| **Btwquestsdragonflight** | `BtWQuestsDragonflight` | 🛑 |
| **Btwquestslegion** | `BtWQuestsLegion` | 🛑 |
| **Btwquestsshadowlands** | `BtWQuestsShadowlands` | 🛑 |
| **Btwqueststhewarwithin** | `BtWQuestsTheWarWithin` | 🛑 |
| **Buttonforge** | `ButtonForge` | 🛑 |
| **Chat Sound Customizer** | `Chat_Sound_Customizer` | 🛑 |
| **Coolinator** | `Coolinator` | ✅ |
| **Datastore** | `DataStore` | 🛑 |
| **Datastore Auctions** | `DataStore_Auctions` | 🛑 |
| **Datastore Characters** | `DataStore_Characters` | 🛑 |
| **Datastore Crafts** | `DataStore_Crafts` | 🛑 |
| **Datastore Garrisons** | `DataStore_Garrisons` | 🛑 |
| **Datastore Inventory** | `DataStore_Inventory` | 🛑 |
| **Datastore Mails** | `DataStore_Mails` | 🛑 |
| **Datastore Quests** | `DataStore_Quests` | 🛑 |
| **Dialogueui** | `DialogueUI` | ✅ |
| **Dynamiccam** | `DynamicCam` | 🛑 |
| **Handynotes** | `HandyNotes` | 🛑 |
| **Handynotes Battleforazeroth** | `HandyNotes_BattleForAzeroth` | 🛑 |
| **Handynotes Cataclysm** | `HandyNotes_Cataclysm` | 🛑 |
| **Handynotes Dragonflight** | `HandyNotes_Dragonflight` | 🛑 |
| **Handynotes Legion** | `HandyNotes_Legion` | 🛑 |
| **Handynotes Mechagonandnazjatar** | `HandyNotes_MechagonAndNazjatar` | 🛑 |
| **Handynotes Mistsofpandaria** | `HandyNotes_MistsOfPandaria` | 🛑 |
| **Handynotes Shadowlands** | `HandyNotes_Shadowlands` | 🛑 |
| **Handynotes Theburningcrusade** | `HandyNotes_TheBurningCrusade` | 🛑 |
| **Handynotes Thewarwithin** | `HandyNotes_TheWarWithin` | 🛑 |
| **Handynotes Visionsofnzoth** | `HandyNotes_VisionsOfNZoth` | 🛑 |
| **Handynotes Warlordsofdraenor** | `HandyNotes_WarlordsOfDraenor` | 🛑 |
| **Handynotes Worldofwarcraft** | `HandyNotes_WorldOfWarcraft` | 🛑 |
| **Handynotes Wrathofthelichking** | `HandyNotes_WrathOfTheLichKing` | 🛑 |
| **Interaction** | `Interaction` | ✅ |
| **Itemversion** | `ItemVersion` | ✅ |
| **Kui Nameplates Core Config** | `Kui_Nameplates_Core_Config` | 🛑 |
| **Leatrix Maps** | `Leatrix_Maps` | 🛑 |
| **Ls Toasts** | `ls_Toasts` | 🛑 |
| **Moveany** | `MoveAny` | 🛑 |
| **Mythicdungeontools** | `MythicDungeonTools` | 🛑 |
| **Narcissus** | `Narcissus` | ✅ |
| **Nys Todolist** | `Nys_ToDoList` | 🛑 |
| **Platynator** | `Platynator` | ✅ |
| **Plumber** | `Plumber` | ✅ |
| **Rematch** | `Rematch` | 🛑 |
| **Scrap** | `Scrap` | ✅ |
| **Shadowedunitframes** | `ShadowedUnitFrames` | 🛑 |
| **Syndicator** | `Syndicator` | ✅ |
| **Tomtom** | `TomTom` | 🛑 |
| **Vaultloom** | `Vaultloom` | ✅ |
| **Warbandworldquest** | `WarbandWorldQuest` | 🛑 |
| **Waypointui** | `WaypointUI` | ✅ |
| **Worldquesttab** | `WorldQuestTab` | ✅ |

---

**Total: 65 translated addons**

## 📋 Translation Status

- ✅ **Complete**: AddOns with full Spanish translations
- 🔄 **In Progress**: AddOns with partial translations  
- ❌ **Pending**: AddOns not yet translated

*Individual addon status may vary. Translations are continuously updated.*
