...
function WarbandWorldQuestNextResetButtonMixin:OnLoad()
	...
	self:SetupMenu(function(_, rootMenu)
		rootMenu:CreateTitle("Excluir misiones de mundo")
...
function WarbandWorldQuestNextResetButtonMixin:Update()
	...
	self.ButtonText:SetText(format("Reinicio: %s (%d)", resetTime and date("%m-%d %H:%M", resetTime) or UNKNOWN, #quests))
...
function WarbandWorldQuestNextResetButtonMixin:OnEnter()
	...
	tooltip:SetText("Próximo reinicio de misiones de mundo", 1, 1, 1)
	tooltip:AddLine(
		format("|cnNORMAL_FONT_COLOR:Tiempo restante:|r |cnWHITE_FONT_COLOR:%s|r", resetTime and Util.FormatTimeDuration(resetTime - GetServerTime()) or UNKNOWN)
	)
	tooltip:AddLine(" ")
	tooltip:AddLine(format("|cnNORMAL_FONT_COLOR:Conteo de misiones:|r |cnWHITE_FONT_COLOR:%d|r", #quests))
...
function WarbandWorldQuestCharactersButtonMixin:OnLoad()
	...
	self:SetupMenu(function(_, rootMenu)
		rootMenu:CreateTitle("Excluir personajes")
...
function WarbandWorldQuestCharactersButtonMixin:RemoveCharacter(character)
	Util:Debug("Removiendo personaje:", character.name)
...
function WarbandWorldQuestCharactersButtonMixin:OnEnter()
	...
	tooltip:SetText("Personajes escaneados", 1, 1, 1)
	tooltip:AddLine(format("|cnNORMAL_FONT_COLOR:Última hora de reinicio:|r |cnWHITE_FONT_COLOR:%s|r", date("%m-%d %H:%M", resetStartTime)))

	for groupName, records in pairs({ ["Pendientes"] = pending, ["Escaneados"] = scanned }) do
		tooltip:AddLine(" ")
...
function WarbandWorldQuestSettingsButtonMixin:Update()
	...
	self:SetupMenu(function(_, rootMenu)
		rootMenu:CreateTitle("Configuración")

		do -- Pines del mapa
			local pinsMenu = rootMenu:CreateButton("Pines del mapa")

			Settings:CreateCheckboxMenu("pins_progress_shown", pinsMenu, "Mostrar progreso de la tropa en los pines")

			do -- Mostrar progreso de la tropa en el tooltip
				local tooltipMenu = Settings:CreateCheckboxMenu("pins_tooltip_shown", pinsMenu, "Mostrar progreso de la tropa en el tooltip")
			...
			pinsMenu:CreateCheckbox(
				"Mostrar pines en los mapas del continente",
			...
			Settings:CreateCheckboxMenu("pins_completed_shown", pinsMenu, "Mostrar pines para misiones completadas")
		end

		do -- Registro de misiones
			local logMenu = rootMenu:CreateButton("Registro de misiones del mundo")
			...
			Settings:CreateCheckboxMenu(
				...
				nil,
				"Establecer |cff00d9ffWarband World Quest|r como la pestaña predeterminada, se abre automáticamente al abrir el mapa del mundo por primera vez después de iniciar sesión"
			)

			Settings:CreateCheckboxMenu(
				"log_scanning_icon_shown",
				logMenu,
				"Mostrar icono de escaneo pendiente",
				nil,
				"Mostrar un icono |A:common-icon-undo:10:10:0:0|a en el título de la misión si el progreso de la misión no se ha escaneado en todos los personajes rastreados"
			)
		end
		...
		do -- Maps
			...
			Settings:CreateMenuTree("maps_to_scan", rootMenu, "Escanear mapas", GetMapName)
		end

		rootMenu:CreateDivider()

		do -- Filtro de recompensas
			rootMenu:CreateTitle("Filtro de recompensas")
...
function WarbandWorldQuestEntryMixin:UpdateTooltip()
	...
	tooltip:AddLine(format("|cnNORMAL_FONT_COLOR:Tiempo restante:|r |cnWHITE_FONT_COLOR:%s|r", Util.FormatTimeDuration(quest.resetTime - GetServerTime())))

	tooltip:AddLine(" ")
	tooltip:AddLine(format("Personajes escaneados: |cnWHITE_FONT_COLOR:%d|r", self.data.progress.total - self.data.progress.unknown))
	tooltip:AddLine(format("Personajes completados: |cnWHITE_FONT_COLOR:%d|r", self.data.progress.claimed))
